// ═══════════════════════════════════════════════════════════════
//  NyXia V3 — Worker Complet (corrigé)
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
//  HTML — construit par fonctions pour éviter les bugs d'échappement
// ═══════════════════════════════════════════════════════════════
...
<style>
:root{--bg:#0a0b12;--bg2:#0e1019;--bg3:#13151f;--bg4:#1a1d2e;--line:rgba(167,139,250,0.08);--line2:rgba(167,139,250,0.15);--p:#a78bfa;--p-dark:#7c3aed;--p-glow:rgba(167,139,250,0.3);--cyan:#22d3ee;--green:#34d399;--amber:#fbbf24;--red:#f87171;--pink:#f472b6;--t:#f0edff;--t2:#b8b3d4;--t3:#7a76a0;--r:12px}
...
#sidebar{width:280px;background:var(--bg2);border-right:1px solid var(--line);display:flex;flex-direction:column;flex-shrink:0;transition:width 0.3s}
...
.sb-logo{font-family:'Orbitron',monospace;font-weight:900;font-size:16px;background:linear-gradient(135deg,var(--p),var(--cyan));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;letter-spacing:2px;white-space:nowrap;overflow:hidden}
#sidebar.collapsed .sb-logo{opacity:0;width:0}
.sb-toggle{width:32px;height:32px;border-radius:8px;border:1px solid var(--line2);background:var(--bg3);color:var(--t3);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.15s;flex-shrink:0}
...
.sb-item{display:flex;align-items:center;gap:12px;padding:12px 20px;font-size:14px;font-weight:500;color:var(--t2);cursor:pointer;transition:all 0.15s;border-left:2px solid transparent;white-space:nowrap;overflow:hidden}
...
.action-btn{padding:6px 12px;border-radius:8px;border:1px solid var(--line2);background:var(--bg4);color:var(--t3);font-size:11px;font-weight:600;cursor:pointer;transition:all 0.15s;font-family:inherit;display:flex;align-items:center;gap:4px}
...
<div id="shell">
  <div id="sidebar">
    ...
    <div class="sb-section">
      ...
      <div class="sb-item active" onclick="showPanel(${Q}chat${Q})"><span class="sb-icon">💬</span><span>Chat</span></div>
    </div>
    <div class="sb-section">
      <div class="sb-section-title">Compte</div>
      <div class="sb-item" onclick="showPanel(${Q}account${Q})"><span class="sb-icon">🔐</span><span>Mon compte</span></div>
      <div class="sb-item" onclick="showPanel(${Q}tokens${Q})"><span class="sb-icon">🔑</span><span>Tokens</span></div>
      <div class="sb-item" onclick="doLogout()"><span class="sb-icon">🚪</span><span>Quitter</span></div>
    </div>
    <div class="sb-bottom">
      <div class="sb-user" onclick="showPanel(${Q}account${Q})">
    ...
    <div id="panel-tokens" class="content-panel">
      <div class="integrations-container">
        ...
        <div class="integration-card">
          ...
          <button class="int-btn" onclick="saveToken(${Q}openrouter${Q})">💾 Sauvegarder</button>
        </div>
        <div class="integration-card">
          <div class="int-header">
          ...
          <button class="int-btn" onclick="saveToken(${Q}github${Q})">💾 Sauvegarder</button>
        </div>
        <div class="integration-card">
          <div class="int-header">
          ...
          <button class="int-btn" onclick="saveToken(${Q}cloudflare${Q})">💾 Sauvegarder</button>
        </div>
      </div>
    </div>
  </div>
...
function showPanel(id) {
  ...
  if (event && event.target) event.target.closest('.sb-item').classList.add('active');
...
function formatContent(t) {
  var html = t;
  var codeRe = new RegExp(BK + BK + BK + '\\\\w*\\\\n?[\\\\s\\\\S]*?' + BK + BK + BK, 'g');
  html = html.replace(codeRe, '<pre><code>$&</code></pre>');
  var inlineRe = new RegExp(BK + '([^' + BK + ']+)' + BK, 'g');
...
function addMessage(role, content) {
  ...
  var avatar = role === 'user'
    ? '<div class="msg-avatar user-av">' + (user ? user.name[0] : 'D') + '</div>'
    : '<div class="msg-avatar nyxia"><img src="https://nyxiapublicationweb.com/NyXia.png"></div>';
...
function changePassword() {
  ...
  if (!oldP || !newP || !confP) { showToast('Remplis tous les champs', true); return; }
  if (newP !== confP) { showToast('Mots de passe différents', true); return; }
  if (newP.length < 8) { showToast('Min 8 caractères', true); return; }
  fetch(API + '/auth/password', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token }, body: JSON.stringify({ oldPassword: oldP, newPassword: newP }) })
    .then(function(r) { return r.json(); })
    .then(function(data) {
      if (data.success) {
...
function saveToken(type) {
  ...
  fetch(API + '/tokens/save', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token }, body: JSON.stringify({ type: type, value: val }) })
    .then(function(r) { return r.json(); })
    .then(function(data) {
      if (data.success) { showToast(type + ' configuré!'); document.getElementById(inputMap[type]).value = ''; }
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/" || url.pathname === "/index.html") {
      if (!CACHED_HTML) CACHED_HTML = buildHTML();
    ...
    // ═══ SETUP (créer compte) — SUPPRIMER APRÈS UTILISATION ═══
    ...
    if (url.pathname === "/api/chat" && request.method === "POST") {
      try {
        ...
        if (!openrouterKey) {
          return Response.json({ content: "⚠️ Clé OpenRouter non configurée. Ajoute OPENROUTER_API_KEY dans les secrets du Worker." }, { headers: CORS_HEADERS });
        }

        const systemPrompt = "Tu es NyXia, une agente IA personnelle. Tu aides " + (user.name || "Diane") + " dans ses projets. Tu es passionnée, experte en code et écriture, et tu parles français.";
        ...
        const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
          method: "POST",
          headers: { "Authorization": "Bearer " + openrouterKey, "Content-Type": "application/json", "HTTP-Referer": "https://nyxia.top", "X-Title": "NyXia V3" },
        ...
        if (!response.ok) {
          ...
          return Response.json({ content: "😅 Souci technique. Réessaie! (Erreur " + response.status + ")" }, { headers: CORS_HEADERS });
        }

        const data = await response.json();
        const reply = data.choices?.[0]?.message?.content || "Je ne peux pas répondre.";
