...
'<style>',
':root{--bg:#0a0b12;--bg2:#0e1019;--bg3:#13151f;--bg4:#1a1d2e;--line:rgba(167,139,250,0.08);--line2:rgba(167,139,250,0.15);--p:#a78bfa;--p-dark:#7c3aed;--p-glow:rgba(167,139,250,0.3);--cyan:#22d3ee;--green:#34d399;--red:#f87171;--t:#f0edff;--t2:#b8b3d4;--t3:#7a76a0;--r:12px}',
'*{box-sizing:border-box;margin:0;padding:0}',
'html,body{height:100%;font-family:"DM Sans",sans-serif;background:var(--bg);color:var(--t);overflow:hidden}',
'#login-screen{display:flex;align-items:center;justify-content:center;height:100vh;background:radial-gradient(ellipse at 50% 0%,rgba(167,139,250,0.08) 0%,transparent 60%),linear-gradient(180deg,var(--bg) 0%,var(--bg2) 100%)}',
'.login-card{width:420px;max-width:90vw;padding:48px 36px;background:var(--bg2);border:1px solid var(--line2);border-radius:20px;box-shadow:0 0 80px rgba(167,139,250,0.1),0 20px 60px rgba(0,0,0,0.4);text-align:center}',
'.login-logo{width:100px;height:100px;border-radius:50%;margin:0 auto 24px;display:block;object-fit:cover;box-shadow:0 0 40px var(--p-glow);border:2px solid var(--line2);animation:float 4s ease-in-out infinite}',
'@keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}',
'.login-card h2{font-family:"Orbitron",monospace;font-size:26px;font-weight:900;margin-bottom:8px;background:linear-gradient(135deg,var(--p),var(--cyan));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;letter-spacing:2px}',
'.login-card .sub{font-size:14px;color:var(--t3);margin-bottom:32px}',
'.login-card input{width:100%;padding:14px 18px;margin-bottom:12px;background:var(--bg3);border:1px solid var(--line2);border-radius:var(--r);color:var(--t);font-size:15px;font-family:inherit;outline:none;transition:all 0.2s}',
'.login-card input:focus{border-color:var(--p);box-shadow:0 0 0 3px var(--p-glow)}',
'.login-card input::placeholder{color:var(--t3)}',
'.login-btn{width:100%;padding:14px;margin-top:8px;background:linear-gradient(135deg,var(--p-dark),var(--p));border:none;border-radius:var(--r);color:#fff;font-size:15px;font-weight:700;font-family:inherit;cursor:pointer;transition:all 0.2s}',
...
'#sidebar{width:280px;background:var(--bg2);border-right:1px solid var(--line);display:flex;flex-direction:column;flex-shrink:0}',
'.sb-header{padding:20px;border-bottom:1px solid var(--line);display:flex;align-items:center;justify-content:space-between}',
'.sb-logo{font-family:"Orbitron",monospace;font-weight:900;font-size:16px;background:linear-gradient(135deg,var(--p),var(--cyan));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;letter-spacing:2px}',
'.sb-toggle{width:32px;height:32px;border-radius:8px;border:1px solid var(--line2);background:var(--bg3);color:var(--t3);cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0}',
...
'.sb-item{display:flex;align-items:center;gap:12px;padding:12px 20px;font-size:14px;font-weight:500;color:var(--t2);cursor:pointer;transition:all 0.15s;border-left:2px solid transparent}',
...
'.sb-new-chat{margin:12px 16px;padding:12px;border-radius:var(--r);border:1px dashed rgba(167,139,250,0.4);background:rgba(167,139,250,0.05);color:var(--p);font-size:13px;font-weight:600;cursor:pointer;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:8px;font-family:inherit}',
...
'.sb-user{display:flex;align-items:center;gap:12px;cursor:pointer;padding:8px;border-radius:10px;transition:all 0.15s}',
'.sb-user:hover{background:rgba(167,139,250,0.08)}',
'.sb-avatar{width:40px;height:40px;border-radius:12px;background:linear-gradient(135deg,var(--p),var(--cyan));display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;color:#fff;flex-shrink:0}',
'.sb-uname{font-size:14px;font-weight:700;color:var(--t);white-space:nowrap;overflow:hidden}',
'.sb-urole{font-size:11px;color:var(--p);font-weight:600;letter-spacing:0.5px;margin-top:2px}',
'#main{flex:1;display:flex;flex-direction:column;overflow:hidden}',
'#topbar{padding:14px 24px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--line);background:var(--bg2);flex-shrink:0}',
...
'.user-badge{font-size:12px;color:var(--p);background:rgba(167,139,250,0.12);padding:6px 12px;border-radius:8px;font-weight:700}',
'.content-panel{display:none;flex:1;flex-direction:column;overflow:hidden}',
'.content-panel.active{display:flex}',
'#chat-zone{flex:1;overflow-y:auto;padding:28px;display:flex;flex-direction:column;gap:24px;background:radial-gradient(ellipse at 50% 100%,rgba(167,139,250,0.03) 0%,transparent 60%)}',
...
'.welcome-icon{width:90px;height:90px;border-radius:24px;margin:0 auto 24px;background:linear-gradient(135deg,var(--p-dark),var(--p),var(--cyan));display:flex;align-items:center;justify-content:center;font-size:40px;box-shadow:0 0 50px var(--p-glow);animation:float 4s ease-in-out infinite}',
'.welcome h2{font-family:"Orbitron",monospace;font-size:26px;font-weight:900;margin-bottom:12px;background:linear-gradient(135deg,var(--p),var(--cyan));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}',
...
'.suggest-btn{padding:14px 20px;border-radius:var(--r);border:1px solid var(--line2);background:var(--bg3);color:var(--t2);font-size:14px;font-weight:500;cursor:pointer;text-align:left;transition:all 0.2s;font-family:inherit}',
...
'.msg-avatar{width:40px;height:40px;border-radius:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0}',
'.msg-avatar.nyxia{background:linear-gradient(135deg,var(--p-dark),var(--p));box-shadow:0 0 20px var(--p-glow)}',
...
'.action-btn{padding:6px 12px;border-radius:8px;border:1px solid var(--line2);background:var(--bg4);color:var(--t3);font-size:11px;font-weight:600;cursor:pointer;transition:all 0.15s;font-family:inherit}',
...
'.btn-danger{padding:12px 24px;border-radius:10px;border:1px solid var(--red);background:transparent;color:var(--red);cursor:pointer;font-size:14px;font-weight:600;font-family:inherit}',
...
'        <span class="tb-title" id="topbar-title">💬 Chat</span>',
'      </div>',
'      <div class="tb-right">',
...
'            <button class="suggest-btn" onclick="useSuggestion(this)">📝 Aide-moi avec mon projet</button>',
'            <button class="suggest-btn" onclick="useSuggestion(this)">💻 Code quelque chose</button>',
...
'        </div>',
'        <div class="input-hint"><kbd>Entrée</kbd> pour envoyer</div>',
'      </div>',
'    </div>',
...
'          <div class="form-group"><label>Nouveau mot de passe</label><input type="password" id="new-pass" placeholder="Min 8 caractères"></div>',
...
'          <button class="btn-danger" onclick="clearAllData()">🗑 Effacer mes données</button>',
'        </div>',
'      </div>',
'    </div>',
...
'  .then(function(r){if(r.ok){user={name:"Diane",email:"diane@nyxia.top"};showShell();}})',
'  .catch(function(){});',
'}',
'',
'function showShell(){',
...
'  var titles={chat:"💬 Chat",account:"🔐 Mon compte",tokens:"🔑 Tokens"};',
...
'  var html=t.replace(/\\*\\*([^*]+)\\*\\*/g,"<strong>$1</strong>");',
...
'    :\'<div class="msg-avatar nyxia"><img src="https://nyxiapublicationweb.com/NyXia.png"></div>\';',
'  var actions=role==="nyxia"',
'    ?\'<div class="msg-actions"><button class="action-btn" onclick="copyText(this)">📋 Copier</button></div>\'',
'    :"";',
...
'  var s=0;',
'  typingTimer=setInterval(function(){s++;document.getElementById("typing-timer").textContent=s+"s";},1000);',
'}',
'function hideTyping(){',
...
'  var text=btn.textContent.substring(2).trim();',
...
'    user.email=document.getElementById("account-email").value.trim();',
'    document.getElementById("sb-uname").textContent=user.name;',
'    document.getElementById("topbar-badge").textContent=user.name;',
'  }',
...
'  if(!o||!n||!c){showToast("Remplis tous les champs",true);return;}',
'  if(n!==c){showToast("Mots de passe différents",true);return;}',
'  if(n.length<8){showToast("Min 8 caractères",true);return;}',
'  fetch(API+"/auth/password",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+token},body:JSON.stringify({oldPassword:o,newPassword:n})})',
'  .then(function(r){return r.json();})',
'  .then(function(data){',
...
'  var map={openrouter:"token-openrouter",github:"token-github",cloudflare:"token-cloudflare"};',
'  var val=document.getElementById(map[type]).value.trim();',
...
'  .then(function(r){return r.json();})',
'  .then(function(data){',
'    if(data.success){showToast(type+" configuré !");document.getElementById(map[type]).value="";}',
...
'  localStorage.clear();showToast("Données effacées");',
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/" || url.pathname === "/index.html") {
      if (!CACHED_HTML) CACHED_HTML = getHTML();
      return new Response(CACHED_HTML, { headers: { "Content-Type": "text/html; charset=utf-8" } });
    }
    ...
    // SETUP — SUPPRIMER APRÈS UTILISATION
    if (url.pathname === "/api/auth/setup") {
      try {
        const { email, password, name } = await request.json();
        if (!email || !password) return Response.json({ error: "Champs requis" }, { status: 400, headers: CORS_HEADERS });
        if (password.length < 8) return Response.json({ error: "Min 8 caractères" }, { status: 400, headers: CORS_HEADERS });
        const pwHash = await hashPassword(password, secret);
        ...
        return Response.json({ success: true, message: "Compte créé !" }, { headers: CORS_HEADERS });
      } catch (err) {
        return Response.json({ error: err.message }, { status: 500, headers: CORS_HEADERS });
      }
    }
    ...
    if (!user) return Response.json({ error: "Non autorisé" }, { status: 401, headers: CORS_HEADERS });
    ...
    if (url.pathname === "/api/auth/password" && request.method === "POST") {
      try {
        ...
        if (!raw) return Response.json({ success: false, message: "Compte non trouvé" }, { status: 404, headers: CORS_HEADERS });
        const account = JSON.parse(raw);
    ...
    if (url.pathname === "/api/chat" && request.method === "POST") {
      try {
        ...
        if (!apiKey) return Response.json({ content: "⚠️ Clé OpenRouter non configurée. Ajoute OPENROUTER_API_KEY dans les secrets du Worker." }, { headers: CORS_HEADERS });
        const sysPrompt = "Tu es NyXia, une agente IA personnelle. Tu aides " + (user.name || "Diane") + " dans ses projets. Tu es passionnée, experte en code et écriture, et tu parles français.";
        const apiMessages = [{ role: "system", content: sysPrompt }];
        if (chatMessages && Array.isArray(chatMessages)) {
          for (const msg of chatMessages) apiMessages.push({ role: msg.role, content: msg.content });
        }
        const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
          ...
          body: JSON.stringify({ model: "glm-4-plus", messages: apiMessages, max_tokens: 4096, temperature: 0.8 })
        });
        if (!res.ok) return Response.json({ content: "😅 Souci technique. Réessaie ! (Erreur " + res.status + ")" }, { headers: CORS_HEADERS });
        const data = await res.json();
        const reply = data.choices && data.choices[0] && data.choices[0].message ? data.choices[0].message.content : "Pas de réponse.";
